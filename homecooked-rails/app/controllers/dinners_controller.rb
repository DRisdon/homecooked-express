class DinnersController < ApplicationController
  ...
  def index_past
    ...
    hosted_dinners = []
    attended_dinners = []
    @hosted_dinners = @current_user.hosted_dinners.where('starts_at < ?', "#{Time.now}").limit(8).order(starts_at: :asc).each do |dinner|
      hosted_dinners << dinner.all_info
    end
    @attended_dinners = @current_user.attended_dinners.where('starts_at < ?', "#{Time.now}").limit(8).order(starts_at: :asc).each do |dinner|
      attended_dinners << dinner.all_info
    end
    render json: {
      hosting: hosted_dinners,
